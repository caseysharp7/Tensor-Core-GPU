...
module Systolic_Array_tb;
    ...
    initial begin
        ...
        for(int i=0; i<8; i++) load_data[i] = 16'h0000;
        ...
        // Load elements in diagonal pattern (systolic array proper loading)
        // Cycle c loads elements along the diagonal where row + col = c
        // load_data[0:3] gets a[0:3][c-0], a[1:3][c-1], a[2:3][c-2], a[3][c-3]
        // load_data[4:7] gets b[c-0][0:3], b[c-1][1:3], b[c-2][2:3], b[c-3][3]
        for(int cycle=0; cycle<10; cycle++) begin
            @(posedge clk);
            
            // Load diagonal elements
            // Left inputs (A): load_data[i] = a[i][cycle-i] if indices valid, else 0
            // Top inputs (B): load_data[4+i] = b[cycle-i][i] if indices valid, else 0
            for(int i=0; i<4; i++) begin
                // Left PE inputs (from matrix A)
                if(cycle-i >= 0 && cycle-i < 4) begin
                    load_data[i] = matrix_a[i][cycle-i];
                end else begin
                    load_data[i] = 16'h0000;
                end
                
                // Top PE inputs (from matrix B)
                if(cycle-i >= 0 && cycle-i < 4) begin
                    load_data[4+i] = matrix_b[cycle-i][i];
                end else begin
                    load_data[4+i] = 16'h0000;
                end
            end
            ...
            for(int i=0; i<4; i++) begin
                if(load_data[i] != 0 || load_data[4+i] != 0) begin
                    $display("  left[%0d]=%d  top[%0d]=%d", i, load_data[i], i, load_data[4+i]);
                end
            end
        end
