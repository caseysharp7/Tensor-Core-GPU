...
module Shared_Memory_Arbiter#(parameter NUM_BANKS = 8, NUM_THREADS = 8)(
    ...
    input logic [2:0] thread_banks [NUM_THREADS-1:0],

    output logic [NUM_THREADS-1:0] threads_en, // per thread enable, not per bank
    ...
    always_ff@(posedge clk) begin
        ...
        else if(warp_stall) begin
            threads_mask <= threads_mask | threads_en;
        end
        else begin
            threads_mask <= '0; // clear the mask when all threads have been served
        end
    end
